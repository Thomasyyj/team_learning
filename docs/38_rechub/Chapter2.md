...
4. 训练方式
   ...
   3. 总结：
      ...
      对于训练数据长尾分布的问题，阿里提出了小批量自适应正则化，通过跳过计算负样本的权重，以及对于出现频率不同的样本加不同权重的方式来解决这个问题，focus更多在出现评率高的样本的同时减小计算量

      对于每层输入数据分布不同的问题，阿里提出了一个船新的自适应激活函数，通过函数均值来平移激活函数，函数方差来改变激活函数的平滑度，从而使得每层的数据都在同一个分布下
